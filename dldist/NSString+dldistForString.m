...
//
//  Work based off of the wiki article: https://en.wikipedia.org/wiki/Damerau–Levenshtein_distance
...
- (unsigned long)getDistanceForInsertionInto:(NSString *)base toMatch:(NSString *)other {
    NSString *otherSubstr = [other substringToIndex:[other length] - 1]; //delete char from other
    unsigned long dist = [base dlDistFromString:otherSubstr];
    return dist + 1; //adding 1 for the insert operation
}

- (unsigned long)getDistanceForDeletionFrom:(NSString *)base toMatch:(NSString *)other {
    NSString *baseSubstr = [base substringToIndex:[base length] -1]; //delete a char from base
    unsigned long dist = [baseSubstr dlDistFromString:other];
    return dist + 1; //adding 1 for the delete operation
}

- (unsigned long)getValueForIndicator:(NSString *)base other:(NSString *)other {
...
- (unsigned long)getDistanceForMatchOrMismatch:(NSString *)base toMatch:(NSString *)other {
...
- (unsigned long) getDistanceForTransposition:(NSString *)base toMatch:(NSString *)other {
...
- (Boolean) isTransposed: (NSString *) base with:(NSString *)other {
...
- (unsigned long) dlDistFromString:(NSString*) other {
    ...
    unsigned long distForInsertion = [self getDistanceForInsertionInto:self toMatch:other];
    unsigned long distForDeletion = [self getDistanceForDeletionFrom:self toMatch:other];
    unsigned long distForMatch = [self getDistanceForMatchOrMismatch:self toMatch:other];
    
    unsigned long levenshteinDist = MIN(distForInsertion, MIN(distForDeletion, distForMatch));
    
    if ([self isTransposed:self with:other]) {
        unsigned long distForTransposition = [self getDistanceForTransposition:self toMatch:other];
